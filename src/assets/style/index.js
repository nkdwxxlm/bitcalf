...
const {primary,success,danger,warning,info,
  primaryRGB,successRGB,dangerRGB,warningRGB,infoRGB,
  ff,ffRGB,fe,feRGB,fc,fcRGB,f9,f9RGB,f6,f6RGB,f3,f3RGB,f0,f0RGB,
  bg,bgRGB,bp,bpRGB,bs,bsRGB,bd,bdRGB,bw,bwRGB,bi,biRGB,
  primaryG,successG,dangerG,warningG,infoG,ffG,feG,fcG,f9G,f6G,f3G,f0G,bgG,bpG,bsG,bdG,bwG,biG
} = Color;
...
* */
const sizes = {desktop: 1199,tablet: 991,phone: 767};
...
const getColor = (c,o,over) => {
  switch (c) {
    case 'p': c=o || o === 0 ? `rgba(${primaryRGB},${'.'+o})` : over ? primaryG : primary; break;
    case 's': c=o || o === 0  ? `rgba(${successRGB},${'.'+o})` : over ? successG : success; break;
    case 'd': c=o || o === 0  ? `rgba(${dangerRGB},${'.'+o})` : over ? dangerG : danger; break;
    case 'w': c=o || o === 0  ? `rgba(${warningRGB},${'.'+o})` : over ? warningG : warning; break;
    case 'i': c=o || o === 0  ? `rgba(${infoRGB},${'.'+o})` : over ? infoG : info; break;
    case 'ff': c=o || o === 0  ? `rgba(${ffRGB},${'.'+o})` : over ? ffG : ff; break;
    case 'fe': c=o || o === 0  ? `rgba(${feRGB},${'.'+o})` : over ? feG : fe; break;
    case 'fc': c=o || o === 0  ? `rgba(${fcRGB},${'.'+o})` : over ? fcG : fc; break;
    case 'f9': c=o || o === 0  ? `rgba(${f9RGB},${'.'+o})` : over ? f9G : f9; break;
    case 'f6': c=o || o === 0  ? `rgba(${f6RGB},${'.'+o})` : over ? f6G : f6; break;
    case 'f3': c=o || o === 0  ? `rgba(${f3RGB},${'.'+o})` : over ? f3G : f3; break;
    case 'f0': c=o || o === 0  ? `rgba(${f0RGB},${`.${o}`})` : over ? f0G : f0; break;
    case 'bg': c=o || o === 0  ? `rgba(${bgRGB},${`.${o}`})` : over ? bgG : bg; break;
    case 'bp': c=o || o === 0  ? `rgba(${bpRGB},${`.${o}`})` : over ? bpG : bp; break;
    case 'bs': c=o || o === 0  ? `rgba(${bsRGB},${`.${o}`})` : over ? bsG : bs; break;
    case 'bd': c=o || o === 0  ? `rgba(${bdRGB},${`.${o}`})` : over ? bdG : bd; break;
    case 'bw': c=o || o === 0  ? `rgba(${bwRGB},${`.${o}`})` : over ? bwG : bw; break;
    case 'bi': c=o || o === 0  ? `rgba(${biRGB},${`.${o}`})` : over ? biG : bi; break;
    default: c = o || o === 0  ? `rgba(${ffRGB},${`.${o}`})` : ''; break;
  }
  return c;
};
...
const getGradient = (bg) => {
  ...
  switch (bg) {
    case 'p':
      gs = primary; ge = primaryG; break;
    case 's':
      gs = success; ge = successG; break;
    case 'd':
      gs = danger; ge = dangerG; break;
    case 'w':
      gs = warning; ge = warningG; break;
    case 'i':
      gs = info; ge = infoG; break;
    case 'ff':
      gs = ff; ge = ffG; break;
    case 'fe':
      gs = fe; ge = feG; break;
...
const flexDirection = (param) => {
  ...
  switch (param) {
    case 'r':
      direction = 'row'; break;
    case 'rs':
      direction = 'row-reverse'; break;
    case 'c':
      direction = 'column'; break;
    case 'cs':
      direction = 'column-reverse'; break;
    default:
      direction = 'row'; break;
  }
...
const justifyContent = (param) => {
  ...
  switch (param) {
    case 'fs':
      content = 'flex-start'; break;
    case 'fe':
      content = 'flex-end'; break;
    case 'c':
      content = 'center'; break;
    case 'sb':
      content = 'space-between'; break;
    case 'sa':
      content = 'space-around'; break;
    default:
      content = 'flex-start'; break;
  }
...
const padding = (props) => {
  ...
  if(p||pt||pr||pb||pl||p === 0||pt === 0||pr === 0||pb === 0||pl === 0){
    ...
    if(typeof pt === 'number' && (pt || pt === 0)){padding += `padding-top:${pt}px;`}
    if(typeof pr === 'number' && (pr || pr === 0)){padding += `padding-right:${pr}px;`}
    if(typeof pb === 'number' && (pb || pb === 0)){padding += `padding-bottom:${pb}px;`}
    if(typeof pl === 'number' && (pl || pl === 0)){padding += `padding-left:${pl}px;`}
  ...
  if(dp||dpt||dpr||dpb||dpl||dp === 0||dpt === 0||dpr === 0||dpb === 0||dpl === 0){
    ...
    if(typeof dp === 'number' && (dp || dp === 0)){
      ...
      if(typeof dpt === 'number' && (dpt || dpt === 0)){css += `padding-top:${dpt}px;`}
      if(typeof dpr === 'number' && (dpr || dpr === 0)){css += `padding-right:${dpr}px;`}
      if(typeof dpb === 'number' && (dpb || dpb === 0)){css += `padding-bottom:${dpb}px;`}
      if(typeof dpl === 'number' && (dpl || dpl === 0)){css += `padding-left:${dpl}px;`}
    }
  ...
  if(tp||tpt||tpr||tpb||tpl||tp === 0||tpt === 0||tpr === 0||tpb === 0||tpl === 0){
    ...
    if(typeof tp === 'number' && (tp || tp === 0)){
      ...
      if(typeof tpt === 'number' && (tpt || tpt === 0)){css += `padding-top:${tpt}px;`}
      if(typeof tpr === 'number' && (tpr || tpr === 0)){css += `padding-right:${tpr}px;`}
      if(typeof tpb === 'number' && (tpb || tpb === 0)){css += `padding-bottom:${tpb}px;`}
      if(typeof tpl === 'number' && (tpl || tpl === 0)){css += `padding-left:${tpl}px;`}
    }
  ...
  if(pp||ppt||ppr||ppb||ppl||pp === 0||ppt === 0||ppr === 0||ppb === 0||ppl === 0){
    ...
    if(typeof pp === 'number' && (pp || pp === 0) ){
      ...
      if(typeof ppt === 'number' && (ppt || ppt === 0)){css += `padding-top:${ppt}px;`}
      if(typeof ppr === 'number' && (ppr || ppr === 0)){css += `padding-right:${ppr}px;`}
      if(typeof ppb === 'number' && (ppb || ppb === 0)){css += `padding-bottom:${ppb}px;`}
      if(typeof ppl === 'number' && (ppl || ppl === 0)){css += `padding-left:${ppl}px;`}
    }
...
const margin = (props) => {
  ...
  if(dm||dmt||dmr||dmb||dml||dm === 0||dmt === 0||dmr === 0||dmb === 0||dml === 0){
    ...
    if(typeof dm === 'number' && (dm || dm === 0) ){
      ...
      if(typeof dmt === 'number' && (dmt || dmt === 0) ){css += `margin-top:${dmt}px;`}
      if(typeof dmr === 'number' && (dmr || dmr === 0) ){css += `margin-right:${dmr}px;`}
      if(typeof dmb === 'number' && (dmb || dmb === 0) ){css += `margin-bottom:${dmb}px;`}
      if(typeof dml === 'number' && (dml || dml === 0) ){css += `margin-left:${dml}px;`}
    }
    combination += `@media (max-width: 1199px){${css}}`;
  }
  if(tm||tmt||tmr||tmb||tml||tm === 0||tmt === 0||tmr === 0||tmb === 0||tml === 0){
    ...
    if(typeof tm === 'number' && (tm || tm === 0)){
      ...
      if(typeof tmt === 'number' && (tmt || tmt === 0)){css += `margin-top:${tmt}px;`}
      if(typeof tmr === 'number' && (tmr || tmr === 0)){css += `margin-right:${tmr}px;`}
      if(typeof tmb === 'number' && (tmb || tmb === 0)){css += `margin-bottom:${tmb}px;`}
      if(typeof tml === 'number' && (tml || tml === 0)){css += `margin-left:${tml}px;`}
    }
    combination += `@media (max-width:991px){${css}}`;
  }
  if(pm||pmt||pmr||pmb||pml||pm === 0||pmt === 0||pmr === 0||pmb === 0||pml === 0){
    ...
    if(typeof pm === 'number' && (pm || pm === 0) ){
      ...
      if(typeof pmt === 'number' && (pmt || pmt === 0) ){css += `margin-top:${pmt}px;`}
      if(typeof pmr === 'number' && (pmr || pmr === 0) ){css += `margin-right:${pmr}px;`}
      if(typeof pmb === 'number' && (pmb || pmb === 0) ){css += `margin-bottom:${pmb}px;`}
      if(typeof pml === 'number' && (pml || pml === 0) ){css += `margin-left:${pml}px;`}
    }
    combination += `@media (max-width:767px){${css}}`;
  }
  if(m||mt||mr||mb||ml||m === 0||mt === 0||mr === 0||mb === 0||ml === 0){
    ...
    if(typeof m === 'number' && (m || m === 0) ){
      ...
      if(typeof mt === 'number' && (mt || mt === 0) ){margin += `margin-top:${mt}px;`}
      if(typeof mr === 'number' && (mr || mr === 0) ){margin += `margin-right:${mr}px;`}
      if(typeof mb === 'number' && (mb || mb === 0) ){margin += `margin-bottom:${mb}px;`}
      if(typeof ml === 'number' && (ml || ml === 0) ){margin += `margin-left:${ml}px;`}
    }
    combination += margin;
  }
  return combination;
...
const column = (props) => {
  ...
  switch (Number(col)) {
    case 1:
      percentage = 8.33333333;break;
    case 2:
      percentage = 16.66666667;break;
    case 3:
      percentage = 25;break;
    case 4:
      percentage = 33.33333333;break;
    case 5:
      percentage = 41.66666667;break;
    case 6:
      percentage = 50;break;
    case 7:
      percentage = 58.33333333;break;
    case 8:
      percentage = 66.66666667;break;
    case 9:
      percentage = 75;break;
    case 10:
      percentage = 83.33333333;break;
    case 11:
      percentage = 91.66666667;break;
    case 12:
...
export const Div = styled.div`
    ...
    ${props => (props['o'] || props['o'] === 0) && typeof props['o'] === 'number' && `opacity:${props['o'] === 0 ? 0 : `0.${props['o']}`};`}      
        
    ${props => props['ch'] &&`
        :hover{color:${getColor(props['ch'],props['cho'])};}
    `} 
    ${props => props['bh'] &&`
...
const getButton = (props,type) => {
  const {xs,s,l,xl,bg,bgo,g,c,co,b,bt,br,bb,bl,bro,i=false} = props;
  let {r} = props;
  let rounded = '';
  ...
  let padding = type === 'button' ? '6px 16px' : '6px',fontSize = '12px',fontWeight = '600',border = `1px solid ${fc};`,
    borderRadius = type === 'button' ? '3px' : '0',lineHeight = '1.42857143';

  if(xs){
    padding = type === 'button' ? '1px 9px' : '1px 3px'; fontWeight = '500'; fontSize = '12px'; lineHeight = i ? '1.4' : '1.5';
  }
  if(s){
    padding = type === 'button' ? '5px 14px' : '5px'; fontSize = '12px'; lineHeight = '1.5';
  }
  if(l){
    padding = type === 'button' ? '8px 20px' : '8px'; lineHeight = '1.33333';
  }
  if(xl){
    padding = type === 'button' ? '12px 28px' : '12px'; fontSize = '16px';lineHeight = '1.33333';
  }
  if(r){
    if(typeof r === 'number'){
      rounded = `${r}px`;
  ...
  return style+`
        ...
        color:${c ? color : bg ? bgo === 0 ? getColor(bg) : ff : f6};
...
export const Button = styled.button`
    :hover {
        ...
        ${props => (props['bgo'] || props['bgo'] === 0) && `
            border:${props.b ? props.b[0] : `1`}px ${(props.b && props.b[1] && props.b[1] === 'd') ? `dotted` : `solid`} ${getColor(props.b ? props.b[2] : props['bg'],null,1)}
    ...
    ${props => props['h'] && typeof props['h'] === 'number' && `height:${props['h']}px;`}
    ${props => props['br'] && typeof props['br'] === 'number' && `border-radius:${props['br']}px;`}
    ${props => props['o'] && typeof props['o'] === 'number' && `opacity:.${props['o']};`}
...
const text = (props) => {
  ...
  if(props && props['ch']){
    res = res+`:hover{color:${getColor(props['ch'],props['cho'])};}`;
  }
  return res;
};
